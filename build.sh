...
if [[ ${PRODUCTION} == "true" ]]; then 
    ...
    #create docker image for linux/amd64
    ${DOCKER} buildx build --load --tag ${REPOSITORY}/go-tiny-mfa:dev-amd64 \
        --platform=linux/amd64  --build-arg ARCH=amd64 \
        --label version=${VERSION} --label commit=${COMMIT} \
        --label revision=${REVISION} --file Dockerfile .
    #create docker image for linux/arm64
    ${DOCKER} buildx build --load --tag ${REPOSITORY}/go-tiny-mfa:dev-arm64 \
        --platform=linux/arm64  --build-arg ARCH=arm64 \
        --label version=${VERSION} --label commit=${COMMIT} \
        --label revision=${REVISION} --file Dockerfile .
    #create docker image for linux/arm
    ${DOCKER} buildx build --load --tag ${REPOSITORY}/go-tiny-mfa:dev-arm   \
        --platform=linux/arm/v6 --build-arg ARCH=arm   \
        --label version=${VERSION} --label commit=${COMMIT} \
        --label revision=${REVISION} --file Dockerfile .

    #push images to registry
    ${DOCKER} push ${REPOSITORY}/go-tiny-mfa:dev-arm
    ${DOCKER} push ${REPOSITORY}/go-tiny-mfa:dev-arm64
    ${DOCKER} push ${REPOSITORY}/go-tiny-mfa:dev-amd64

    #create new :latest manifest 
    ${DOCKER} manifest create \
                ${REPOSITORY}/go-tiny-mfa:development \
                ${REPOSITORY}/go-tiny-mfa:dev-amd64 \
                ${REPOSITORY}/go-tiny-mfa:dev-arm64 \
                ${REPOSITORY}/go-tiny-mfa:dev-arm
